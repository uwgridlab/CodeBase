function [rsig, rsta] = resynchGugerData(sig, sta, chansPerAmp, chansToIgnore)
    ...
    % it's unnecessary to filter all the channels, just need to do the ones
    % to compare
    asig = bandpass(double(sig(:,:)), 50, 70, 1200, 4);
    
    fsig = asig(:,keeperChannels);
    ...
    %     ampShifts = [];
        ...
        for ampNum = 2:nAmps
            ...
            if (~isempty(compLags(:)))
                lag = mode(compLags(:)); % changed from median, why was this median?
                if (lag > 0)
                    warning('lag greater than zero, unable to shift');
                    lag = 0;
                end
