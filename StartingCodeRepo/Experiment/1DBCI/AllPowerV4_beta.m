...
cacheFile = fullfile(pwd, 'metadata', 'AllPower_beta.m.cache', [subject '.mat']);
...
for recnum = 1:length(ds.recs)
    ...
    if(recnum == 1 && strcmp(dsFile, fullfile('metadata', 'ds', 'fc9643_ud_mot_t_ds.mat'))  == 1) 
        controlChannel = 23;
        warning('little hack here');
    elseif (strcmp(dsFile, fullfile('metadata', 'ds', 'fc9643_ud_mot_t_ds.mat'))  == 1)
        controlChannel = 24;
    end
...
%    signals = NotchFilter(signals, [60 120 180], parameters.SamplingRate);
    signals = BandPassFilter(signals, [12 30], parameters.SamplingRate, 4);
...
%     if(sum(sum(isnan(epochZ))) || sum(sum(isnan(restZ))))
%         x = 5;
%     end
...
clearvars -except Montage badchans cRange controlChannel ds dsFile epochZs outputDir restZs resultCodes targetCodes side subject cacheFile version
save(cacheFile);