...
for c = 1:length(subjids)
    ...
    load(['..\metadata\AllPower_beta.m.cache\' subjids{c} '.mat']);
...
%     downareas     = cat(1, downareas, montageAreas(downh == 1));
    ...
    save(fullfile(cacheOutDir, ['shift_' subjids{c} '_beta.mat']), 'allh', 'uph', 'downh');
...
%     'PPC', 'Other', 'class failure'}, 'YTick', 1:7);
    ...
    SaveFig(figOutDir, sprintf('prepostshift_beta.class.%s',direction), 'png');    
end
...
%     set(cb, 'YTickMode', 'manual');
    SaveFig(figOutDir, sprintf('prepostshift_beta.tal.%s',direction), 'png');    
end
...
%             
    SaveFig(figOutDir, sprintf('prepostshift_beta.bar.%s',direction), 'eps');
end
...
colorbar;
SaveFig(figOutDir, 'prepostshift_beta.gauss', 'png');
