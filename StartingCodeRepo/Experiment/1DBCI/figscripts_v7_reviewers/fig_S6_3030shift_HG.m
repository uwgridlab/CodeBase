...
%     [~, ~, div] = getBCIFilesForSubjid(subjids{c});
    ...
    load(['..\metadata\AllPower.m.cache\' subjids{c} '.mat']);
    ...
    ldiv = round(.3*length(targetCodes));
    hdiv = round(.7*length(targetCodes));
    
    preCodes = targetCodes(1:ldiv);
    postCodes = targetCodes(hdiv:end);
    
    preZs = epochZs(:, 1:ldiv);
    postZs = epochZs(:, hdiv:end);
...
%             [downrsas, downh] = signedSquaredXCorrValue(postdownZs, predownZs, 2, 0.05/size(epochZs,1));
            [allrsas, allh] = signedSquaredXCorrValue(postZs, preZs, 2, 0.05/(chanct - badct));    
            [uprsas, uph] = signedSquaredXCorrValue(postupZs, preupZs, 2, 0.05/(chanct - badct));
            [downrsas, downh] = signedSquaredXCorrValue(postdownZs, predownZs, 2, 0.05/(chanct - badct));
        case 2
            allrsas = mean(postZs,2)-mean(preZs,2);
%             allh = (ttest2(postZs', preZs', 0.05/size(epochZs,1)) == 1);
            [allh, p] = ttest2(postZs', preZs', 0.05/(chanct-badct));
            allh = allh==1;
            
            uprsas = mean(postupZs,2)-mean(preupZs,2);
%             uph = (ttest2(postupZs', preupZs', 0.05/size(epochZs,1)) == 1);
            uph = (ttest2(postupZs', preupZs', 0.05/(chanct - badct)) == 1);

            downrsas = mean(postdownZs,2)-mean(predownZs,2);
%             downh = (ttest2(postdownZs', predownZs', 0.05/size(epochZs,1)) == 1);
            downh = (ttest2(postdownZs', predownZs', 0.05/(chanct - badct)) == 1);
    end
    ...
    fprintf('for subject (%s): min = %e, max = %e\n', subjids{c}, min(p(allh)), max(p(allh)));
...
%     downareas     = cat(1, downareas, montageAreas(downh == 1));
    ...
    save(fullfile(cacheOutDir, ['shift_' subjids{c} '_3030.mat']), 'allh', 'uph', 'downh');
...
%     'PPC', 'Other', 'class failure'}, 'YTick', 1:7);
    ...
    SaveFig(figOutDir, sprintf('prepostshift_3030.class.%s',direction), 'png');    
end
...
%     set(cb, 'YTickMode', 'manual');
    SaveFig(figOutDir, sprintf('prepostshift_3030.tal.%s',direction), 'png');    
end
...
%             
    SaveFig(figOutDir, sprintf('prepostshift_3030.bar.%s',direction), 'eps');
end

%% do the single gauss plot

load(fullfile(getSubjDir('tail'),'surf','tail_cortex_rh_hires.mat'))

vals = allrsas_vals;
locs = allrsas_locs;
locs(:,1) = abs(locs(:,1));

ctmr_gauss_plot(cortex, locs, vals, 'r', [-2.5 2.5], 1, 'recon_colormap_rev');
% ctmr_gauss_plot(cortex, locs, vals, 'r', [-max(abs(vals)) max(abs(vals))], 1, 'recon_colormap_rev');
colorbar;
SaveFig(figOutDir, 'prepostshift_3030.gauss', 'png');
