...
for c = 1:length(subjids)
    ...
    [uSigs, uPs] = ttest2(uZs', restZs', 0.05 / (chanct-badct), 'right', 'unequal');
    uSigs = uSigs' == 1;
    [dSigs, dPs] = ttest2(dZs', restZs', 0.05 / (chanct-badct), 'right', 'unequal');
    dSigs = dSigs' == 1;
    [aSigs, aPs] = ttest2(epochZs', restZs', 0.05 / (chanct-badct), 'right', 'unequal');
    aSigs = aSigs' == 1;
...
colormap(cm(32:end,:));
colorbar;
circleControlTrodes(gca, subjids(uCtlChanFlag_all==1), true);
SaveFig(figOutDir, 'overall.tal.up.lg', 'png', '-r600');
...
colormap(cm(32:end,:));
colorbar;
circleControlTrodes(gca, subjids(dCtlChanFlag_all==1), true);
SaveFig(figOutDir, 'overall.tal.down.lg', 'png', '-r600');
...
colormap(cm(32:end,:));
colorbar;
circleControlTrodes(gca, subjids(aCtlChanFlag_all==1), true);
SaveFig(figOutDir, 'overall.tal.all.lg', 'png', '-r600');
