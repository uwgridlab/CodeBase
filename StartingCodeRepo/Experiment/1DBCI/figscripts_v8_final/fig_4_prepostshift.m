...
%     % collect trode locations in talairach space
    if (exist([getSubjDir(subjids{c}) 'tail_trodes.mat'], 'file'))
        load([getSubjDir(subjids{c}) 'tail_trodes.mat']);
    else
        load(fullfile(getSubjDir(subjids{c}), 'other', 'tail_trodes.mat'));
    end
    ...
    load(fullfile(getSubjDir(subjids{c}), 'other', 'brod_areas.mat'));
    ...
    for electrodeElt = Montage.MontageTokenized
        ...
        eval(sprintf('numitems = size(%s,1);', label{:}));
        values = hmatValueForElectrodes(AllTrodes(loc:(loc+numitems-1),:));
...
%     downareas     = cat(1, downareas, montageAreas(downh == 1));
    ...
    allprezs = cat(1, allprezs, mean(preZs(allh == 1, :),2));
    allpostzs = cat(1, allpostzs, mean(postZs(allh == 1, :),2));
    
    TouchDir(cacheOutDir);
    save(fullfile(cacheOutDir, ['shift_' subjids{c} '.mat']), 'allh', 'uph', 'downh');
...
%             
    SaveFig(figOutDir, sprintf('prepostshift.bar.%s',direction), 'eps', '-r900');
end
...
SaveFig(figOutDir, 'prepostshift.tal.gauss', 'png', '-r600');
