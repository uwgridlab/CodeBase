...
cachefile = fullfile(cacheOutDir, ['fig_bytime_beta.' subjid '.mat']);
...
if (exist(cachefile, 'file'))
    ...
    for c = 1:length(files)
        ...
        signalsHilbAmp = hilbAmp(signals, [12 30], parameters.SamplingRate.NumericValue);
...
% %     otherwise use this section
    tit = sprintf('%s - average responses ', id);
...
%     mtit(tit, 'xoff', 0, 'yoff', .03, 'FontSize', big, 'FontName', 'Arial');
    ...
    SaveFig(figOutDir, ['bytime_beta.' subjid '.' cleantname '.lg'], 'eps', '-r900');    
    close all;
