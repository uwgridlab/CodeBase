...
if (~exist('doPlots', 'var') || doPlots == true)     
    ...
    PlotDotsDirect(subjid, trodeLocsFromMontage(subjid,Montage,false), ...
        uvals, side, [-1 1], 20, 'recon_colormap');
    title(sprintf('%s RSAs (fb v r) for all sessions, up targets', subjid));
    colormap(cm);
    colorbar;

    SaveFig(fullfile(pwd, 'figs'), ['overall.' subjid '.up.sm']);
    maximize;
    SaveFig(fullfile(pwd, 'figs'), ['overall.' subjid '.up.lg']);
    
    figure;
    PlotDotsDirect(subjid, trodeLocsFromMontage(subjid,Montage,false), ...
        dvals, side, [-1 1], 20, 'recon_colormap');
    title(sprintf('%s RSAs (fb v r) for all sessions, down targets', subjid));
    colormap(cm);
    colorbar;
    
    SaveFig(fullfile(pwd, 'figs'), ['overall.' subjid '.down.sm']);
    maximize;
    SaveFig(fullfile(pwd, 'figs'), ['overall.' subjid '.down.lg']);
end
