...
for c = 1:length(subjids)
    ...
    uvals_all = cat(1, uvals_all, uvals);
    dvals_all = cat(1, dvals_all, dvals);
    tlocs_all  = cat(1, tlocs_all,  tlocs);
    
    uvals_scrn = cat(1, uvals_scrn, uvals(~isnan(uvals)));
    dvals_scrn = cat(1, dvals_scrn, dvals(~isnan(dvals)));
    ulocs_scrn = cat(1, ulocs_scrn, tlocs(~isnan(uvals),:));
    dlocs_scrn = cat(1, dlocs_scrn, tlocs(~isnan(dvals),:));
    
    clearvars -except uvals_all dvals_all tlocs_all uvals_scrn dvals_scrn ulocs_scrn dlocs_scrn subjids c;
end
