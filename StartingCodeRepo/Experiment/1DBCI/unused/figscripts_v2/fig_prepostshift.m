...
%% transformation
% order of idxs: M1/S1 PMd PMv BA8/9/45/46 PPC Other Fail
...
allIdxs = {
    ...
    find(allbareas == 7)
    find(allfareas == 0 & ~ismember(allbareas, [8 9 46 7]) & ~isnan(allbareas))
...
upIdxs = {
    ...
    find(upbareas == 7)
    find(upfareas == 0 & ~ismember(upbareas, [8 9 46 7]) & ~isnan(upbareas))
    find(isnan(upbareas))
    };
...
downIdxs = {
    ...
    find(downbareas == 7)
    find(downfareas == 0 & ~ismember(downbareas, [8 9 46 7]) & ~isnan(downbareas))
...
%     'PPC', 'Other', 'class failure'}, 'YTick', 1:7);
    ...
    SaveFig(fullfile(pwd, 'figs'), sprintf('prepostshift.class.%s',direction), 'png');    
end
...
%     set(cb, 'YTickMode', 'manual');
    SaveFig(fullfile(pwd, 'figs'), sprintf('prepostshift.tal.%s',direction), 'png');    
end
