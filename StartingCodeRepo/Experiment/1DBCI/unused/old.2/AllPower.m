...
if(true)%~exist(cacheFile, 'file'))
    ...
    for recnum = 1:length(ds.recs)
        ...
        maxlen = min(length(restOnset),length(restOffset));
        restOnset = restOnset(1:maxlen);
        restOffset = restOffset(1:maxlen);
...
%         fbOnset = find(fbEvents == 1) - round(0.5*parameters.SamplingRate);
    ...
    %     scores = (sigavs - mean(restavs)) / mean(reststds);
        ...
        allsigs = cat(2, allsigs, epochSignals);
        allscores = [allscores; scores];
        
        save(['temp2_' num2str(recnum)], 'scores','allscores');
