function [rs, outputFilepath] = TrialByTrialVariance(ds)
% bci dmn analysis    
    rs.aggregate.trodeStatus = [];
    rs.aggregate.epochs = [];
    rs.aggregate.targets = [];
    rs.aggregate.variances = [];
    ...
    for recNum = 1:length(ds.recs)
        ...
        rs.aggregate.variances = cat(2, rs.aggregate.variances, rs.results(recNum).variances);
    ...
    outputFilename = ['TrialByTrialVariance_' ds.subjId '_' ds.type '_rs'];
...
function result = processRecording(rec)
    ...
    signals = bandpass(signals, 70, 170, fs, 4);
...
%     [starts, ends] = getEpochs(targetCode > 0 & feedback == 0 & resultCode == 0, 1);
    [starts, ends] = getEpochs(feedback == 1 & resultCode == 0, 1);
    ...
    result.epochs = getEpochSignal(hamp, starts, ends);
    result.targets = targetCode(ends);
    result.results = resultCode(ends + 1);
    result.variances = squeeze(var(result.epochs, 0, 1));
