function out = jacketWindowedCov(a,b,windowSize, prePostLag, samplesToSkip)

    a = gdouble(a);
    b = gdouble(b);

    valueIndices = gones(prePostLag*2+1, windowSize+1);
    ...
    out = gzeros(numWindows,prePostLag*2+1);

    for outIdx = 1:numWindows
        values = gones(size(valueIndices,1),size(valueIndices,2)+1); 
