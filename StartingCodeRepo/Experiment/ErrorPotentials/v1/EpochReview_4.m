%% this script selects which electrodes 
subjid = 'fc9643';

%% load data
load(fullfile(subjid, [subjid '_decomp.mat']), 'tgts', 'ress', 'fw' , 't', 'Montage', 'bads');
...
for chan = 1:max(cumsum(Montage.Montage))
    if (~ismember(chan, bads))
        ...
        load(fullfile(subjid, 'chan', [num2str(chan) '.mat']));
        dca = abs(decompAll);
        ...
        load(fullfile(subjid, 'stats', [num2str(chan) '.mat']));
        ...
        if (~isempty(cClusters))
            ...
            SaveFig(fullfile(pwd, subjid, 'fig'), [num2str(chan) '.eps'], 'eps');
...
save(fullfile(subjid, 'significant_clusters.mat'), 'clusterChans', 'allClusters', 'masks', 'allRsas', 'p_targ');