...
if (isfield(Montage, 'Default') == false || Montage.Default == false) % actually have a montage, do the cortical plots
    if (aggregate == true)
        figure;
        PlotDots(subjid, Montage.MontageTokenized, HGRSAs(1, :), 'l', [-1 1], 20, 'recon_colormap');
        load('recon_colormap');
        colormap(cm);
        ...
        figure;
        PlotDots(subjid, Montage.MontageTokenized, BetaRSAs(1, :), 'l', [-1 1], 20, 'recon_colormap');
        load('recon_colormap');
        colormap(cm);
        ...
        for activityIdx = 1:length(activities)
            figure;
            PlotDots(subjid, Montage.MontageTokenized, HGRSAs(activityIdx, :), 'l', [-1 1], 20, 'recon_colormap');
            load('recon_colormap');
            colormap(cm);
            ...
            figure;
            PlotDots(subjid, Montage.MontageTokenized, BetaRSAs(activityIdx, :), 'l', [-1 1], 20, 'recon_colormap');
            load('recon_colormap');
            colormap(cm);
