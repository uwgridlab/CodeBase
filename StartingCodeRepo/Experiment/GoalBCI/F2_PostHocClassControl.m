...
%%% do post-hoc classification analyses, on the control subject only
%% (1) extract features
allFeatures = cell(1, 1);
allLabels = cell(1, 1);
c_allLabels = cell(1, 1);
...
save(fullfile(META_DIR, 'post-hoc-classification-control.mat'), ...
    'allFeatures',   'allLabels',   'accs',   'gammas',   'cs',   'estimates',   'posteriors',   'featureRanks', ...
                 ...
                 %%
load(fullfile(META_DIR, 'post-hoc-classification-control.mat'), ...
    'allFeatures',   'allLabels',   'accs',   'gammas',   'cs',   'estimates',   'posteriors',   'featureRanks', ...
                     'c_allLabels', 'c_accs', 'c_gammas', 'c_cs', 'c_estimates', 'c_posteriors', 'c_featureRanks');
...
title('Post-hoc classification of visual control');
...
xlim([.5 1.5]);
legend('actual', 'rand. control', 'chance', '95% CI', 'location', 'southeast');

SaveFig(OUTPUT_DIR, 'ph class vcontrol', 'png', '-r300');
SaveFig(OUTPUT_DIR, 'ph class vcontrol', 'eps', '-r300');
...
%     [p_fdr, p] = fdr(p, 0.05);
    ...
    figure
    PlotDots(SIDS{sidx}, montage.MontageTokenized, r2, determineHemisphereOfCoverage(SIDS{sidx}), [-max(r2) max(r2)], 20, 'america');
...
bas = brodmannAreaForElectrodes(trodeLocsFromMontage('979eab', montage, true), false);
