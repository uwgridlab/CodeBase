...
for c = 1:length(SIDS)
    ...
    for e = 1:length(paths)
        ...
        error{e}    = diffs{e};
        interaction{e} = error{e} .* velocity{e};
...
%         direction{e} = targetY{e}(start:endd) > .5; % up is 1, down is zero
    ...
    txts = {{velocity, 'velocity'}, {error, 'error'}, {interaction, 'interaction'}};
    for sub = txts
        qty = sub{1}{1};
        txt = sub{1}{2};
                
        [mpred, ~] = reshapeCellPredictors(qty', data, lags, DO_PREROLL);                        
        predictors = cat(2,predictors,mpred);
    end
    
    [temp, outcomes] = reshapeCellPredictors(direction', data, lags, DO_PREROLL);        
    predictors(:, end+1) = temp(:, lags==0);    
    ...
    coeffs = zeros(length(lags)*length(txts)+1, size(data, 1));
    ...
    save(fullfile(META_DIR, ['lasso ' subjid '.mat']), 'outcomes', 'predictors', 'lags', 'coeffs', 'mses', 'predictions', 'stats', ...
