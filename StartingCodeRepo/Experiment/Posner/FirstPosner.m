% % file = 'C:\Research\Data\Patients\cde503\d6\cde503_posner001\cde503_posnerS001R05.dat';
% % 
...
for i=1:length(flashAt)-1
    ...
    if(val < 1200*(1.05) && val > 1200*(0.95))
        fprintf ('-Arrow');
    elseif(val < 600*(1.05) && val > 600*(0.95))
        fprintf ('-Cross');
    end
...
catchTrials = [22 34 35 41 42];
colors = 'krgb';
figure
for recChan =1:64
    subplot(8,8,recChan);
    for period =4:4 
        plot((period-1)*45 + [1:45],exp(meanPower(:,recChan,period)),[colors(period) '.']);
        hold on;
        plot((period-1)*45 + catchTrials, exp(meanPower(catchTrials,recChan,period)),'markersize',10,'marker','s', 'color',[colors(period)],'linestyle','none');
        hold on;
    end
end
DensePlot(8,8);
...
for i=1:6
    ...
    switch (condition)
        ...
        case 2
            title('Cross vs ITI');
        case 3
            title('Response vs ITI');
        case 4
            title('Cross vs Arrow');
        case 5
            title('Response vs Arrow');
        case 6
            title('Response vs Cross');
    end
    view(-10,0);
