...
for c = 1:length(SIDS);
    ...
    [~, odir] = filesForSubjid(subjid, META_DIR);
    load(fullfile(META_DIR, [subjid '_epochs.mat']), 'bad_marker', 'tgts', 'ress');

    prefoo(c) = length(tgts);
...
% 
SaveFig(OUTPUT_DIR, 'task_performance', 'png', '-r600');
