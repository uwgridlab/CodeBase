...
%         if (~isempty(which('gpuDevice')))
%             hg = extractWhitenedHG_GPU(sig, fs, sta.TargetCode==0);
%         else
%             hg = extractWhitenedHG(sig, fs, sta.TargetCode==0);
%         end
        
        hg = hilbAmp(sig, [70 150], fs);
        hg = zscore(hg);
        
        beta = hilbAmp(sig, [12 24], fs);
        beta = zscore(beta);
        
        alpha = hilbAmp(sig, [8 12], fs);
        alpha = zscore(alpha);
...
%         beta = zscoreAgainstInterest(beta, sta.TargetCode, 0);
        ...
        fsFinal = 400;        
        DSFAC = fs/fsFinal;        
        t = interp1(1:length(t), t, linspace(1, length(t), length(t)/DSFAC));
...
%             samples = mode(pends-rstarts+1);
            ...
            mepochs_alpha = zeros(size(sig,2), length(rstarts), samples);
...
%                 mepochs_alpha(:, e_ctr, :) = alpha(e,:)'; 
                mepochs_alpha(:, e_ctr, :) = resample(alpha(e,:), fsFinal, fs)';
            ...
            epochs_alpha = cat(2, epochs_alpha, mepochs_alpha);
        end
    ...
    % save the rest
    save(fullfile(META_DIR, [sid '_epochs']), 'montage', 'bads', 'epochs_*', '*Dur', 'fs', 'hemi', 'ress', 'src_files', 't', 'tgts', 'cchan', 'bad_marker', 'bad_channels');
