...
% sig is TxChan
    ...
    for chan = 1:size(sig, 2)
        ...
        if (~any(chan==bads))
            gchansig = (sig(:,chan));
            call_down = (zeros(length(centers), nfft));
            gwin = (repmat(hann(winWidth), 1));
            gnfft = (nfft);
...
%                     gtempwin = gpuArray(repmat(hann(length(cRange)), 1));
                    ...
                    temp = gchansig(cRange).*gwin;
                    call_down(centerIdx, :) = fft(temp, gnfft);            
                end        
            end
...
%             end
            ...
            bb_down(:, chan) = (nspectra * filters(:, 1));
