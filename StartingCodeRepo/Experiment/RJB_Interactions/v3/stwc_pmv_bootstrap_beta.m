...
for sIdx = 1:length(SIDS)
    ...
    load(fullfile('meta', sprintf('%s_extracted.mat', sid)), 'c_beta', 'p_hg', 'trs', 't', 'fs', 'tgts', 'ress', '*Dur', 'cchan');
    ...
    if (DO_XTRODE_HG)        
        ...
        for chanIdx = 1:size(p_hg, 1)
            ...
            for n = 1:N                
                interaction = zeros(2*maxLag + 1, tkeepN, size(c_beta, 2));
                
                for epochIdx = 1:size(c_beta, 2)
                    sc = scramblePhase(c_beta(:, epochIdx));
