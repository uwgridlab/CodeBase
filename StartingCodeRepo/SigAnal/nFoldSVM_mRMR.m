function [hits, counts] = nFoldSVM_mRMR(features, labels, n, svmMethod, nFeats, c, gamma)
    if (~exist('svmMethod', 'var'))
        svmMethod = 'matlab';
    end
    
    if (~exist('nFeats','var') || isempty('nFeats'))
        nFeats = 10;
    end
    ...
    for i = 1:CVO.NumTestSets
        ...
        dfeatures = discretizeFeatures(features(:, trIdx));
        featureIdxs = mrmr_miq_d(dfeatures', double(labels(trIdx)), nFeats);
        mrmrFeatures = dfeatures(featureIdxs, :);
